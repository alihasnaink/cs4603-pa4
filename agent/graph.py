...
def load_mcp_tools(server_path: str | None = None) -> list[Any]:
    # Must patch BEFORE importing langchain_mcp_adapters/mcp: mcp's
    # stdio_client() binds errlog=sys.stderr as a function *definition*-time
    # default. If sys.stderr is Databricks' StreamToLogger (no fileno()) at
    # import time, that broken reference is baked in permanently.
        ...
        client = MultiServerMCPClient(
            {
                "analyst": {
                    "command": python_exe,
                    "args": ["-u", server_path],   # -u = unbuffered stdout, fixes the hang
                    "transport": "stdio",
                }
            }
        )
        return await asyncio.wait_for(client.get_tools(), timeout=30)  # fail loud, not silent
...
def _real_std_streams():
    orig_out, orig_err = sys.stdout, sys.stderr
    devnull_out = open(os.devnull, "w")
    devnull_err = open(os.devnull, "w")
    try:
        if not hasattr(orig_out, "fileno"):
            sys.stdout = devnull_out
        if not hasattr(orig_err, "fileno"):
...
def _run_async(coro):
    ...
    def _target():
        ...
        try:
            with _real_std_streams():
                result["value"] = loop.run_until_complete(coro)
        except* Exception as eg:
            inner = eg.exceptions[0] if hasattr(eg, "exceptions") else eg
            logger.error("MCP tool loading failed: %r", inner, exc_info=inner)
            result["error"] = inner
        finally:
            loop.close()
